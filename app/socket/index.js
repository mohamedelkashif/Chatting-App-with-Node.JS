			...
			Room.findOne({'title': new RegExp('^' + title + '$', 'i')}, function(err, room){
				...
				if(room){
					...
					Room.create({ 
						...
						// socket.emit('updateallUsersList', users, true);
						// socket.broadcast.emit('updateUsersList', newRoom);
						// socket.broadcast.to(newRoom.id).emit('updateallUsersList', users[users.length - 1]);
					});
				}
			});
		});
